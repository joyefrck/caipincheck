...
**对话式偏好调整 (Update & Redirect)**:
用户可以通过自然语言调整偏好，系统会**立即更新权重并重定向当前推荐意图**：
- 输入"我更喜欢川菜" → 🛠️ 背景更新川菜权重 -> 🔄 立即为您呈上一套川菜大餐
- 输入"不要那么辣" → 🛠️ 降低辣味权重 -> 🔄 实时切换为清淡/不辣的推荐
- 输入"多一些海鲜" → 🛠️ 提高海鲜权重 -> 🔄 搜索库内海鲜菜品进行搭配
- 返回的菜谱卡片中会带有偏好更新成功的即时反馈标识。
