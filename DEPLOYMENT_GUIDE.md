# 🚀 生产环境部署指南 - 食谱模型进化更新

## 📋 更新概述

本次更新引入了用户画像系统和智能推荐引擎，涉及数据库Schema变更和新增功能。

### 主要变更
- ✅ 新增 `user_profile` 表（用户画像）
- ✅ 新增 `user_feedback` 表（用户反馈记录）
- ✅ `base_recipes` 表新增4个字段（cooking_methods, nutrition_tags, taste_tags, cuisine_type）
- ✅ 新增对话式偏好调整功能
- ✅ 新增智能推荐API

---

## ⚠️ 部署前检查

### 1. 环境要求
- Node.js >= 18.0
- npm >= 9.0
- SQLite3
- Docker & Docker Compose（如使用容器）

### 2. 必需的环境变量
确保 `.env` 或 Docker 环境中配置了：
```bash
DEEPSEEK_API_KEY=<your-api-key>
TZ=Asia/Shanghai
```

---

## 📦 部署步骤

### 步骤 1: 备份生产数据库 ⭐️

**在服务器上执行**:

```bash
# 进入项目目录
cd /path/to/caipincheck

# 创建备份目录
mkdir -p backups

# 手动备份数据库（带时间戳）
cp food-check.db backups/food-check.db.backup_$(date +%Y%m%d_%H%M%S)

# 验证备份
ls -lh backups/
```

**⚠️ 重要**: 备份成功后再继续下一步！

---

### 步骤 2: 拉取最新代码

```bash
# 拉取最新代码
git pull origin main

# 或者从 GitHub 拉取
git fetch origin
git checkout main
git pull
```

---

### 步骤 3: 安装依赖

```bash
# 安装新增的依赖（如果有）
npm install
```

---

### 步骤 4: 执行数据库迁移 🔧

#### 方案 A: 使用自动化脚本（推荐）

```bash
# 进入脚本目录
cd server/scripts

# 执行迁移（自动处理所有操作）
node migrate_db.js
```

**预期输出**:
```
🔧 开始执行数据库迁移...

📋 步骤 1/3: 执行迁移脚本
  ✅ 迁移脚本执行完成

📋 步骤 2/3: 验证表结构
  ✅ 表 user_profile 已创建，包含 7 个字段
  ✅ 表 user_feedback 已创建，包含 5 个字段
  ✅ base_recipes.cooking_methods 字段已添加
  ✅ base_recipes.nutrition_tags 字段已添加
  ✅ base_recipes.taste_tags 字段已添加
  ✅ base_recipes.cuisine_type 字段已添加

📋 步骤 3/3: 为现有菜谱补充标签数据
  📊 找到 XXX 条现有菜谱
  ✅ 已为 XXX 条菜谱补充标签数据

🎉 数据库迁移全部完成！
```

#### 方案 B: 手动执行 SQL（备选）

如果自动化脚本失败，可以手动执行：

```bash
# 进入项目目录
cd /path/to/caipincheck

# 使用 SQLite3 执行迁移脚本
sqlite3 food-check.db < server/migrations/001_user_profile.sql
```

---

### 步骤 5: 存量食谱数据初始化 🍜

#### 5.1 自动标注策略

迁移脚本会自动为存量食谱补充标签：

**标注规则**:

| 字段 | 推断逻辑 |
|-----|---------|
| `cuisine_type` | 根据现有 `tags` 字段（川菜、湘菜等）或标题关键词 |
| `taste_tags` | 根据标题关键词（"辣"→辣、"甜"→甜） |
| `cooking_methods` | 根据标题关键词（"炸"→油炸、"蒸"→蒸煮） |
| `nutrition_tags` | 根据食材类型（肉类→蛋白质、蔬菜→维生素） |

**示例**:
```javascript
标题: "麻辣水煮鱼"
已有标签: ["川菜", "海鲜"]

自动推断结果:
- cuisine_type: "川菜"
- taste_tags: ["辣", "麻"]
- cooking_methods: ["煮", "炖煮"]
- nutrition_tags: ["蛋白质"]
```

#### 5.2 验证标注结果

```bash
# 查询标注统计
sqlite3 food-check.db << EOF
SELECT 
  COUNT(*) as total,
  COUNT(CASE WHEN cuisine_type != '' THEN 1 END) as has_cuisine,
  COUNT(CASE WHEN taste_tags != '[]' THEN 1 END) as has_taste,
  COUNT(CASE WHEN cooking_methods != '[]' THEN 1 END) as has_cooking
FROM base_recipes;
EOF
```

**预期输出**:
```
total|has_cuisine|has_taste|has_cooking
500  |450        |380       |420
```

#### 5.3 手动补充（可选）

如果自动标注覆盖率不足，可以使用AI批量标注：

```bash
# 创建 AI 标注脚本
node server/scripts/ai_enrich_recipes.js
```

**脚本功能**:
- 对缺失标签的菜谱，使用 DeepSeek AI 分析标题和食材
- 自动生成菜系、口味、烹饪方法、营养标签
- 批量更新数据库

---

### 步骤 6: 重启服务

#### Docker 部署

```bash
# 重新构建并启动服务
docker compose up -d --build

# 查看日志
docker compose logs -f
```

#### PM2 部署

```bash
# 重启应用
pm2 restart caipincheck

# 查看日志
pm2 logs caipincheck
```

#### 1Panel 部署

1. 进入 1Panel 控制台
2. 找到 caipincheck 容器
3. 点击 "重建" 或 "重启"
4. 查看容器日志确认启动成功

---

### 步骤 7: 功能验证 ✅

#### 7.1 验证数据库

```bash
# 检查表结构
sqlite3 food-check.db << EOF
.schema user_profile
.schema user_feedback
PRAGMA table_info(base_recipes);
EOF
```

#### 7.2 验证API

```bash
# 1. 测试用户画像接口
curl http://your-domain:3001/api/user-profile/peter_yong

# 2. 测试智能推荐接口
curl -X POST http://your-domain:3001/api/recommend \
  -H "Content-Type: application/json" \
  -d '{"userId": "peter_yong", "diners": 2}'

# 3. 测试对话式偏好调整（在前端对话框输入）
# "我更喜欢川菜"
```

#### 7.3 验证前端

访问应用，测试以下功能：
- [ ] "主厨今日特供"能够返回推荐结果
- [ ] 点击"喜欢"按钮后权重更新
- [ ] 在对话中输入"我更喜欢川菜"能得到确认消息
- [ ] 收藏功能正常
- [ ] 历史记录正常

---

## 🔄 回滚方案

如果部署后出现问题，执行以下步骤回滚：

### 1. 停止服务

```bash
docker compose down
# 或
pm2 stop caipincheck
```

### 2. 恢复数据库

```bash
# 找到最新备份
ls -lt backups/

# 恢复数据库
cp backups/food-check.db.backup_YYYYMMDD_HHMMSS food-check.db
```

### 3. 回滚代码

```bash
# 回退到上一个版本
git log --oneline -10
git checkout <previous-commit-hash>
```

### 4. 重启服务

```bash
docker compose up -d
# 或
pm2 restart caipincheck
```

---

## 📊 部署后监控

### 1. 数据库增长监控

```bash
# 每天检查数据库大小
du -h food-check.db

# 监控菜谱数量增长
sqlite3 food-check.db "SELECT COUNT(*) FROM base_recipes;"
```

### 2. API 响应时间

```bash
# 测试推荐接口响应时间
time curl -X POST http://localhost:3001/api/recommend \
  -H "Content-Type: application/json" \
  -d '{"userId": "peter_yong", "diners": 2}'
```

### 3. 日志监控

```bash
# Docker
docker compose logs -f --tail=100

# PM2
pm2 logs caipincheck --lines 100
```

---

## 🛠️ 常见问题

### Q1: 迁移脚本报错 "duplicate column name"

**原因**: 字段已存在（可能已执行过迁移）

**解决**: 这是正常的警告，脚本会自动跳过已存在的字段

---

### Q2: 存量菜谱标签未生成

**排查步骤**:
1. 检查菜谱是否有 `title` 和 `tags` 字段
2. 查看迁移脚本日志
3. 手动执行标注：
   ```bash
   node server/scripts/migrate_db.js
   ```

---

### Q3: 用户画像不存在

**解决**:
```bash
# 手动创建用户画像
sqlite3 food-check.db << EOF
INSERT OR IGNORE INTO user_profile (user_id, taste_weights, cuisine_weights, ingredient_weights, cooking_method_weights, nutrition_weights, updated_at)
VALUES (
  'peter_yong',
  '{"甜": 0.5, "咸": 0.5, "酸": 0.5, "苦": 0.5, "辣": 0.5, "鲜": 0.5, "麻": 0.5}',
  '{"川菜": 0.5, "粤菜": 0.5, "鲁菜": 0.5, "湘菜": 0.5, "苏菜": 0.5, "浙菜": 0.5, "徽菜": 0.5, "闽菜": 0.5}',
  '{}',
  '{"油炸": 0.5, "煎炒": 0.5, "炖煮": 0.5, "蒸煮": 0.5, "烤制": 0.5, "凉拌": 0.5, "煲汤": 0.5}',
  '{"蛋白质": 0.5, "脂肪": 0.5, "碳水化合物": 0.5, "维生素": 0.5, "矿物质": 0.5, "膳食纤维": 0.5}',
  strftime('%s', 'now') * 1000
);
EOF
```

---

### Q4: DeepSeek API 调用失败

**排查**:
1. 检查环境变量 `DEEPSEEK_API_KEY` 是否配置
2. 验证 API Key 是否有效
3. 检查网络连接

---

## 📝 部署检查清单

部署前打印此清单，逐项确认：

- [ ] 已备份生产数据库
- [ ] 已拉取最新代码
- [ ] 已安装依赖 `npm install`
- [ ] 已执行数据库迁移 `node migrate_db.js`
- [ ] 验证表结构创建成功
- [ ] 验证存量菜谱标签已补充
- [ ] 已重启服务
- [ ] API 接口测试通过
- [ ] 前端功能测试通过
- [ ] 已配置日志监控

---

## 🎯 预期结果

部署成功后，您将获得：

1. ✅ **用户画像系统** - 自动学习用户偏好
2. ✅ **智能推荐引擎** - 7天不重复，多样性保障
3. ✅ **对话式偏好调整** - 自然语言调整口味
4. ✅ **存量数据标注** - 现有菜谱自动补充标签
5. ✅ **向后兼容** - 原有功能完全保留

---

## 📞 技术支持

如遇到问题，可查看：
- [用户画像系统设计](./user_profile_design.md)
- [推荐算法详解](./recommendation_algorithm.md)
- [README 完整文档](./README.md)

或联系开发团队获取支持。
