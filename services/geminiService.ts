...
2. 锁定偏好：本轮优先参考【${prefs.cuisine || '中餐'}】架构 (特别是：${prefs.subCuisine || '香辣'})。
...
4. 核心决策优先级（绝对执行）：
   ...
   - 【第四优先级 (兜底)】：只有当用户输入极其模糊且未指定口味或形式时，才执行【辣的/湘菜】默认风格。
5. 严格回避：绝对禁止生成以下方案：${excludedTitles.join(', ')}。

关键约束：
- 需求理解：你要能听懂“南瓜，汤”这样的组合指令。既然用户提到了“汤”，你就必须给出南瓜浓汤、南瓜排骨汤等汤类做法，严禁给出像“蛋黄焗南瓜”这种干煸/炒制的菜。
- 菜品配置：${prefs.diners} 人就餐，必须提供 ${prefs.diners >= 3 ? '至少 3 个（其中必须包含 1 个汤品，形成“两菜一汤”或“多菜一汤”的稳稳搭配）' : isMultiDish ? '至少 2 个' : '1 个'} 菜品搭配。
